...
# 데이터베이스 생성 시도
try:
    # 기본 접속용 test_db를 매개로 연결해 CREATE DATABASE 수행
    temp_url = f"mysql+pymysql://{user}:{password}@{endpoint}:{port}/test_db"
...
for filepath in schedule_files:
    ...
    # 헤더가 없는 CSV이므로 명시적으로 지정
    df = pd.read_csv(filepath, header=None, names=['date', 'time', 'away_team', 'home_team', 'stadium', 'status'], encoding="utf-8-sig")
